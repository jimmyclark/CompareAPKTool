...
RichText.debugBg = function(self, bDebug)
	if bDebug then
		self.m_Bg = new(Image, "common/subViewEdit.png")
...
RichText.__splitText = function(self, index, cur_line_width, extra_char, isImage, str_left)
	...
	if not isImage then
		...
		local sub_str1 = utf8_substring(str, 1, math.floor(self.m_width/self.m_fontSize*2)-extra_char)
		-- 安全处理，因为上一步不能保证截取出来的sub_str1一定比str短
		if string.len(sub_str1) == string.len(str) then
			sub_str1 = utf8_sub(sub_str1, 1, utf8_len(sub_str1)-1)
		end
