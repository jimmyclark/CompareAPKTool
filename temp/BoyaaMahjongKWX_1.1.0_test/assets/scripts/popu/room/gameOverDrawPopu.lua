local GameOverDrawPopu = class(require("popu.gameWindow"))
...
GameOverDrawPopu.path = "popu/room/gameOver/win/"

function GameOverDrawPopu:ctor()
end

function GameOverDrawPopu:initView(data)
	...
	moneyNode:hide()
	local viewNode = self:findChildByName("view_draw")
...
-- function GameOverDrawPopu:onShowEnd()
-- 	self.super:onShowEnd()
	
-- end

function GameOverDrawPopu:onClickDeatailBtn()
	self.m_closeFlag = true
    WindowManager:showWindow(WindowTag.GameOverDetailPopu, self.m_gameData)
    self:dismiss()
end

function GameOverDrawPopu:requestReady()
...
function GameOverDrawPopu:dismiss(directFlag)
	GameOverDrawPopu.super.dismiss(self, directFlag)
	-- GuideManager:stop()
	return true
end

function GameOverDrawPopu:onHidenEnd(...)
	GameOverDrawPopu.super.onHidenEnd(self, ...)
