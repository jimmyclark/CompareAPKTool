local GameOverWinPopu = class(require("popu.gameWindow"))
...
GameOverWinPopu.path = "popu/room/gameOver/win/"

function GameOverWinPopu:ctor()
end

function GameOverWinPopu:initView(data)
	...
	local viewNode = self:findChildByName("view_win")
...
function GameOverWinPopu:onClickDeatailBtn()
	printInfo("GameOverWinPopu:onClickDeatailBtn")
	self.m_closeFlag = true
    WindowManager:showWindow(WindowTag.GameOverDetailPopu, self.m_gameData)
    self:dismiss()
end

function GameOverWinPopu:requestReady()
...
function GameOverWinPopu:dismiss(directFlag)
	GameOverWinPopu.super.dismiss(self, directFlag)
	-- GuideManager:stop()
	return true
end

function GameOverWinPopu:onHidenEnd(...)
	GameOverWinPopu.super.onHidenEnd(self, ...)
