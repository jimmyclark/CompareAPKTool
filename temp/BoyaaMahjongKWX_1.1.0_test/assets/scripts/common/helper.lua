...
function playOutCardSound(card, sex)
	local soundPrefix = sex == 0 and "ManCard_" or "WomanCard_"
	local name = ""..(math.floor(card / 16))
	name = name..(card % 16)
	local fileName = soundPrefix .. tostring(name);
...
function playPiaoAndLiangDaoSound(_type, sex)
	if not _type or not sex then return end
	local sexPrefix = sex == 0 and "Man" or "Woman"
	local fileName
	if 1 == _type then
		fileName = sexPrefix .. "Piao"
	elseif 2 == _type then
		fileName = sexPrefix .. "LiangDao"
	end
	local soundTb = Effects.mutiEffectMap[GameSetting:getSoundType()]
	if fileName and soundTb and soundTb[fileName] then
...
function checkOperateData(operation, cardValue, anGang, buGang, liangInfo)
		...
		elseif type(value) == "table" then
			...
			for k, soundTb in pairs(value) do
				for soundType, name in pairs(soundTb) do
					if soundType == SoundType.KWXMJ and PhpManager:getGdmjCanUse() ~= 1 then
						break
					end
